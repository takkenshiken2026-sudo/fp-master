...
## 計算を間違えないためのルール（数値・税務用語）

相続税の基礎控除・法定相続分・贈与税など、**計算が絡む用語**は次を必ず守ります。

1. **人数を先に確定**し、その後 `3,000万＋600万×人数` を計算する（逆算しない）。
2. **金額の名称**（課税価格の合計額／基礎控除額／課税遺産総額）を混同しない。
3. **本文・例題・FAQ** で同じ家族構成なら同じ人数・同じ金額にする。
4. 早見表・具体例は [fp2-glossary-calculation-rules.md](./fp2-glossary-calculation-rules.md) を参照。
...
`tools/glossary_term_rules.py` が人数と金額の不整合を WARN します。
