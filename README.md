...
ファイナンシャル・プランナー試験（FP2級・FP3級）の学習サイト。**FPマスター**（https://fp-master.jp）

`exam-site-shell` テンプレートから派生。共通エンジンの正本は `~/Projects/exam-site-shell`。サイト名・試験名・分野・CSV を差し替え、`python3 tools/build_all.py` で SPA・静的 SEO ページ・配布用バンドルを生成します。
